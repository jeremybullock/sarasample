...
GraphWidget::GraphWidget(QWidget* parent)
  ...
  m_director->setGraphWidget(this);
}
...
void GraphWidget::setPoints(QList<QPointF> points) {
...
QList<QPointF> GraphWidget::getPoints() {
  QList<QPointF> points;
...
bool GraphWidget::removeCurrentControlPoint() {
  return removeControlPoint(m_currentControlPointIndex);
}

//-----------------------------------------------------------------------------

bool GraphWidget::middlePointSelected() {
    if (m_currentControlPointIndex >= m_points.size() - 5) {
        return false;
    }
    if (m_currentControlPointIndex <= 4) {
        return false;
...
bool GraphWidget::removeControlPoint(int index) {
  ...
  m_currentControlPointIndex = -1; // firstIndex - 2;
...
void GraphWidget::mouseMoveEvent(QMouseEvent* e) {
  ...
  m_director->setLastClicked(Director::PATHPOINT);
}

...
public:
  ...
  QPoint m_liveViewZoomOffset     = QPoint(0, 0);
  bool m_liveViewZoomReadyToPick  = true;
  QPointF m_liveViewZoomPickPoint = QPointF(0.0, 0.0);
  QPoint m_zoomRectDimensions     = QPoint(0, 0);
  QPoint m_finalZoomPoint         = QPoint(0, 0);
  QRect m_zoomRect                = QRect(0, 0, 0, 0);
  QSize m_proxyImageDimensions = QSize(0, 0);
  bool m_converterSucceeded         = false;
  bool m_pickLiveViewZoom           = false;
  QPoint m_fullImageDimensions  = QPoint(0, 0);
...
// Canon Commands
  ...
  void makeZoomPoint(QPointF pos);

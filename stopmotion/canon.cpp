...
void Canon::resetCanon(bool liveViewOpen) {
  m_proxyImageDimensions = QSize(0, 0);
...
bool Canon::downloadImage(EdsBaseRef object) {
  ...
  if (isRaw) {
    ...
    if ((m_imageQuality.toLower().contains("jpeg") ||
      ...
      return err;
      StopMotion::instance()->m_tempRaw = "C:/temp";
    }
  ...
  QSize res(1920, 1080);
  if (m_proxyImageDimensions == QSize(0, 0)) {
  ...
  int new_y = (double)m_proxyImageDimensions.width() / r;
  int new_x = (double)m_proxyImageDimensions.width();
  cv::Size dim(new_x, new_y);
  ...
  StopMotion::instance()->m_newImage = QImage(newWidth, newHeight, QImage::Format_ARGB32);
  uchar* rawData = StopMotion::instance()->m_newImage.bits();
  memcpy(rawData, resizedBuf, newSize);


  m_proxyImageDimensions = StopMotion::instance()->m_newImage.size();
  ...
  if (!isRaw) {
    ...
    dataStream.writeRawData((const char*)rawStreamData, rawStreamSize);
    fullImage.close();
    JpgConverter::saveJpg(StopMotion::instance()->m_newImage, (StopMotion::instance()->m_tempFile));
  }
...
EdsError Canon::zoomLiveView() {
  ...
  Commands::instance()->getAction("Focus")->setChecked(m_liveViewZoom > 1);
  return err;
}

//-----------------------------------------------------------------

void Canon::makeZoomPoint(QPointF pos) {
...
void Canon::toggleZoomPicking() {
  if (m_sessionOpen && StopMotion::instance()->m_liveViewStatus > 0 &&
    ...
    if (m_pickLiveViewZoom) {
      ...
      if (m_liveViewZoomPickPoint == QPointF(0.0, 0.0)) {
        makeZoomPoint(m_liveViewZoomPickPoint);
      }
    }
  }
  emit(pickFocusCheckToggled(m_pickLiveViewZoom));
  Commands::instance()->getAction("Pick Focus")->setChecked(m_pickLiveViewZoom);
}
...
void Canon::calculateZoomPoint() {
    QSizeF size = QSizeF(1920.0, 1080.0);
    QPoint calculatedZoomPoint;
    bool outOfBounds = false;

    double imgHeight = m_fullImageDimensions.y();
    double imgWidth = m_fullImageDimensions.x();
    double ratio = (double)imgWidth / (double)imgHeight;
    double newHeight = (double)1920.0 / ratio;

    // if a point isn't set, use the center of the image
    // zoom point is the top left corner of the zoom box
    if (m_liveViewZoomPickPoint == QPointF(0.0, 0.0)) {
        calculatedZoomPoint =
            QPoint(m_fullImageDimensions.x() / 2, m_fullImageDimensions.y() / 2);
        m_finalZoomPoint.setX(calculatedZoomPoint.x() - (m_zoomRectDimensions.x() / 2));
        m_finalZoomPoint.setY(calculatedZoomPoint.y() - (m_zoomRectDimensions.y() / 2));
    }
    else {
        // convert the pick point to an image position;
        // the viewer deals with 1080p dimensions;
        double newX = m_liveViewZoomPickPoint.x() / 1920.0 * (double)m_fullImageDimensions.x();
        double newY = m_liveViewZoomPickPoint.y() / (double)newHeight * (double)m_fullImageDimensions.y();

        // the Canon SDK wants the top left corner of the zoom rect, not the center
        m_finalZoomPoint.setX(newX - (m_zoomRectDimensions.x() / 2));
        m_finalZoomPoint.setY(newY - (m_zoomRectDimensions.y() / 2));
        // finally make sure everything actually fits on the image
        if (m_finalZoomPoint.x() < 0) {
            m_finalZoomPoint.setX(0);
            outOfBounds = true;
        }
        if (m_finalZoomPoint.y() < 0) {
            m_finalZoomPoint.setY(0);
            outOfBounds = true;
        }
        if (m_finalZoomPoint.x() >
            m_fullImageDimensions.x() - (m_zoomRectDimensions.x())) {
            m_finalZoomPoint.setX(m_fullImageDimensions.x() - (m_zoomRectDimensions.x()));
            outOfBounds = true;
        }
        if (m_finalZoomPoint.y() >
            m_fullImageDimensions.y() - (m_zoomRectDimensions.y())) {
            m_finalZoomPoint.setY(m_fullImageDimensions.y() - (m_zoomRectDimensions.y()));
            outOfBounds = true;
        }

        // if out of bounds, recalculate backwards to get the correct location
        if (outOfBounds) {
            QPoint tempCalculated;
            // recenter the point in the rect
            tempCalculated.setX(m_finalZoomPoint.x() + (m_zoomRectDimensions.x() / 2));
            tempCalculated.setY(m_finalZoomPoint.y() + (m_zoomRectDimensions.y() / 2));

            //m_liveViewZoomPickPoint.setX(tempCalculated.x());
            //m_liveViewZoomPickPoint.setY(tempCalculated.y());
        }
    }

    // calculate the zoom rectangle position on screen
    m_zoomRect =
        QRect(m_finalZoomPoint.x(), m_finalZoomPoint.y(), m_zoomRectDimensions.x(), m_zoomRectDimensions.y());
    setZoomPoint();

    double screenX = (((double)m_finalZoomPoint.x() + (m_zoomRectDimensions.x() / 2)) / (double)m_fullImageDimensions.x()) * 1920.0;
    double screenY = (((double)m_finalZoomPoint.y() + (m_zoomRectDimensions.y() / 2)) / (double)m_fullImageDimensions.y()) * newHeight;
    m_liveViewZoomPickPoint.setX(screenX);
    m_liveViewZoomPickPoint.setY(screenY);
}
...
bool Canon::downloadEVFData() {
  ...
  if (err == EDS_ERR_OK) {
    ...
    m_fullImageDimensions = QPoint(coordSys.width, coordSys.height);
    // sometimes zoomRect returns the wrong dimensions if called too fast
    if (zoomRect.size.width < 5000 && zoomRect.size.height < 5000) {
      m_zoomRectDimensions = QPoint(zoomRect.size.width, zoomRect.size.height);
      if (m_zoomRect == QRect(0, 0, 0, 0)) {
        m_zoomRect = QRect(zoomPos.x, zoomPos.y, zoomRect.size.width, zoomRect.size.height);
        m_finalZoomPoint.setX(zoomPos.x);
        m_finalZoomPoint.setY(zoomPos.y);
        calculateZoomPoint();
      }
    }
    if (zoomAmount == 5 && m_zoomRectDimensions == QPoint(0, 0)) {
      setZoomPoint();
    }

...
QImage Webcam::getWebcamQImage() {
  ...
  if (!error) {
    ...
    tempImage = QImage(width, height, QImage::Format_ARGB32);
    // m_liveViewImage = TRaster32P(width, height);

    uchar* imgBuf  = imgCorrected.data;
    uchar* rawData = tempImage.bits();
    memcpy(rawData, imgBuf, size);
    m_realWebcamHeight = height;
    m_realWebcamWidth = width;
    return tempImage;
...
void Webcam::refreshWebcamResolutions() {
  ...
  QList<QSize> tempResolutions = getWebcam()->cameraDevice().photoResolutions();
  m_webcamResolutions.clear();
  for (auto res : tempResolutions) {
      if (m_webcamResolutions.indexOf(res) == -1) {
          m_webcamResolutions.push_back(res);
      }
  }
  if (cameras.size() > 1 || m_webcamResolutions.size() == 0) {
      m_webcamResolutions.clear();

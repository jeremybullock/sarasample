...
void JpgConverter::convertFromJpg() {
  ...
  m_finalImage = QImage(width, height, QImage::Format_RGB32);
  // m_finalImage->lock();
  uchar* rawData = m_finalImage.bits();
...
void JpgConverter::saveJpg(QImage image, QString path) {
...
bool JpgConverter::loadJpg(QString path, QImage &image) {
  long size;
